...
class SViT(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.block = nn.ModuleList(
            [SBlock(args.hidden_dim, args.num_heads, is_snn=args.snn, rate=args.rate, trs=args.trs) for _ in
             range(self.depths)])

        self.lif = neuron.LIFNode(tau=2.0, detach_reset=True) if args.snn else nn.ReLU()

        # classification head
        self.head = layer.Linear(int(args.hidden_dim * args.rate),
    ...
    def forward(self, x):
        ...
        sequence_0_1 = x
        sequence_0_1 = sequence_0_1.mean(dim=0)
        x = self.head(x)
